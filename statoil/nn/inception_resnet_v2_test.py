...
from statoil.nn.inception_resnet_v2 import build_model
...
def test_inception_resnet_v2():
    ...
    model.fit(
        ...
        callbacks=[TensorBoard(log_dir='/tmp/inception_resnet_v2/run-{}'.format(datetime.utcnow().strftime("%Y%m%d%H%M%S")))])
